...
### Q-072 · Which project owns the harness now that `electrical-build/` exists?
**Ask:** `02-PROJECTS/electrical-build/` (2026-09-01) restates the harness —
design, shopping list, install — for isolated readers, and it is already
*ahead* of this tree: its `01-DESIGN/data/pins.csv` resolves items this
project's `pmu_pins.csv` leaves open (A3 = brake + wiper park, 4 states;
A8 = hazard · horn · wink L · wink R summed; +5 V plate-only with 10 kΩ
pull-ups in the legs; O2/O3 as filament). None of that is logged here, and
neither tree has the 2026-09-02 cart decisions (family-colour solid wire,
no backer plate, DMWD wink buttons, no ATC assortment). Two owners for one
fact (R3) — and the build is about to follow one of them.
**Options:**
(A) `electrical-build` owns design / shopping / install; this project keeps
what the sponsor folder deliberately omits — `03-MODULES` + firmware,
`05-PROCESS` (this log, OPEN, TASKS, BOM money, tools), `STATUS`. Superseded
`01-DESIGN`, `02-HARNESS`, `04-BUILD`, `panel-sheet.svg` and BOM §11 move to
`99-ARCHIVE/2026-09-01_electrical-pmu-harness/`; `gen.py` retires (its
outputs go with them; `channels.h` stays as a static file); links in this
project's process files are rewritten to the archive path in one scripted
pass; `check.py` keeps running on both projects.
(B) This tree stays owner; `electrical-build` is a dated export — every
change is made twice, by hand, since the sponsor tables have no generator.
(C) Archive this whole project — strands ~140 hrs of firmware, the
append-only history and the only money ledger.
**Recommendation:** (A), in this order: log the deltas first (D-212 wire
scheme, D-213 no plate / open mounting, D-214 the presentation's design
defaults adopted, pin by pin), then move, then rewrite links, then update
`STATUS.md` §5 (T-053 figures are stale: the carts are DeutschConnector
$944 · Amazon $1,096 · Waytek $455 · WireBarn $383, no Ballenger line).
**Blocks:** nothing physical. One AUDIT session, no money.
...
`Q-048` is a catalogue search. Design context: [`TAIL-LIGHTS.md` (archived)](../../../99-ARCHIVE/Electrical/2026-08-31_lighting-body/TAIL-LIGHTS.md).
...
| V-066 | 7-inch round or rectangular sealed beams on this car — and whether LED housings are already fitted ([`../01-DESIGN/LOADS.md`](../01-DESIGN/LOADS.md) says they are; [`TAIL-LIGHTS.md` (archived)](../../../99-ARCHIVE/Electrical/2026-08-31_lighting-body/TAIL-LIGHTS.md) §8 assumed round sealed beams) | Look in the bucket | `T-035` |
