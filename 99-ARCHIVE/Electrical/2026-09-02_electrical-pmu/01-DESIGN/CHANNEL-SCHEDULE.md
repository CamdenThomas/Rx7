...
`soft_fuse`, run `../05-PROCESS/tools/gen.py`, and this table, [`SPEC.md`](SPEC.md),
...
|---|---|---|
| PMU client config | The soft-fuse column, entered by hand at [`CHECKLIST.md`](../04-BUILD/CHECKLIST.md) 2.22 | Per channel, as each measurement arrives |
...
| [`../04-BUILD/MIGRATION-LOG.md`](../04-BUILD/MIGRATION-LOG.md) | The per-circuit target to compare against | Read from here |
...
That is the whole reason `T-014` → D-197 is the one irreversible-window task: every
...
**Parasitic draw, factory harness, 2026-08: effectively zero** — a brief
60 mA on clamping, settling to 0 on the 2 A range after 20 minutes asleep;
at or below the meter's drift floor. The finished car gets compared against
this at shakedown.
