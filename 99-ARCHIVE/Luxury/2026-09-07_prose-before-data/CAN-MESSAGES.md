...
*Rev 2026-09-07 · owns: the CAN2 message IDs, byte layouts, rates and timeout behaviour. `firmware/icu/can_map.h` is the machine-readable copy; when this file changes, that header and both firmware versions bump together.*
...
(D-106); `0x218` was added 2026-09-07 (D-306) and `can_map.h` has not yet been
bumped for it — see §6. The PMU messages (0x100–0x130) are **intent** until `V-065` — the
...
|---|---|---|---|
| PMU-24 DL | 0x100–0x13F | keypad, DCU requests; logs ICU 0x200 / 0x218 — never a control source (electrical D-251) | **Software, at the dash end** |
...
**Added 2026-09-07 (D-306).** Fuel level left the PMU when A7 was re-pointed at
the oil-pressure node (electrical D-249); the ICU taps the gauge node on
`DP-ICU-B 3` and publishes it here. Deliberately *not* in `0x200`: a slow body
signal does not belong in the 20 Hz engine frame, and keeping it in its own
higher-ID frame is what makes oil pressure outrank fuel level on the bus —
priority is per frame, never per byte.
...
| 1 | Blower duty | uint8, % 0–100 — what the DCU is commanding its final stage at `L3-BLW` (D-308); telemetry for the cluster, the PMU does not act on it |
...
| 5 | Outside-air temp | int8, °C — sensor on `L2-S 5` via `DP-DCU 2` (electrical D-256); shown beside cabin temp |
| 6 | reserved | |
| 7 | counter | |
...
| 0x218 | 3 s | Blank the fuel field, not last value; the PMU logs a timeout as a received-channel fault |
| 0x300 / 0x310 | 2 s | Climate display shows "—"; the blower is unaffected — its final stage is on a local wire from the DCU, not on this bus (D-308) |
...
**Single source of truth per signal** (D-078 → D-306). If the PMU measures it, the ICU
reads it from CAN — battery voltage, key state, output states, channel currents.
Fuel level is the ICU's (electrical D-249 / D-306). Never fit a second sender for
something already measured; the one deliberate double read is the oil-pressure
node (gauge, PMU A7, ICU), as a cross-check.

**Wire is control, CAN is telemetry** (electrical D-251). Nothing on this bus is
ever a PMU control source; a CAN value admitted to a PMU rule fails open on
timeout. Priority (lower ID) buys latency under contention, not availability.
...
fails entirely, only battery voltage goes blank — fuel level is an ICU-local input since D-306.
...
- [ ] Bump `can_map.h` and both firmware versions for `0x218` and the reserved
      `0x120` bytes 0–1 (D-306), and for `0x300` bytes 1 and 5 (D-307) — this file is ahead of the header until then
