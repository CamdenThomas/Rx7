...
| F8 | — | Sill node, sealed inline holder fitted — no fuse this build | Window branch DRV — the luxury package drops the fuse in (D-283) | O1 via L4-P 3 (capped) | EMPTY |
| F9 | — | Sill node, sealed inline holder fitted — no fuse this build | Window branch PASS — the luxury package drops the fuse in (D-283) | O1 via L4-P 3 (capped) | EMPTY |
...
| F14 | — | Sill node, sealed inline holder fitted — no fuse this build | Mirror heat — the luxury package drops the fuse in (D-283) | O15 via L4-P 4 (capped) | EMPTY |
...
| O1 splice | Receptacle L4-P 3 | 12 | RED | Window bus — lands in the sill node's F8 / F9 holders, fitted empty (D-283); no load until the windows (D-274) |
| PMU pin 27 (O15) | Splice → receptacles L3-P 2 and L4-P 4 | 12 | RED | Comfort feeds with no load this build — the dash's ends in the L3-CMF receptacle (D-274), the sill's in F14's holder, fitted empty (D-283, D-254); no fuse block this build |
...
| 3 | Window motor bus feed | O1 (pin 38) splice | 12 | RED | CAPPED | Lands at the sill node — the K5–K8 contact feed through the F8 / F9 holders, fitted empty (D-283); the node is this branch's end (D-274) |
| 4 | Comfort feed → sill (mirror heat F14) | O15 (pin 27) splice | 12 | RED | CAPPED | Lands at the sill node in F14's holder, fitted empty (D-283) — this branch's end (D-274); the DCU switches mirror heat through it later (D-254, restores D-181) |
...
| Sill | **Sill node** — New panel behind the driver kick panel | D1 / D2 receptacles → pinned on the node side, door plugs filled with sealing plugs (RESERVED, D-274)<br>Ground stud → chassis<br>K5–K8 sockets → fitted, empty<br>F8 / F9 / F14 → sealed inline holders fitted, no fuse this build (D-283) | — |
