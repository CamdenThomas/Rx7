...
One kind of item only: a question is anything not yet settled — a call only the owner can make, a fact to confirm, or a measurement to take. Answering one turns it into a `D-` entry in [`DECISIONS.md`](DECISIONS.md), and it leaves this file. IDs are permanent; the ones that came from the old verify/assumption lists keep their old numbers, new ones run from Q-100.
...
{{count:questions|status=open|section=1}} open items. `T-017` is a desk check and has to be done before the design freezes. `Q-113`, `Q-114` and `Q-134`–`Q-139` come from the 2026-09-05 and 2026-09-08 sweeps; each carries a recommendation and wants one word. `Q-128` wants a source and the nRF Connect look. Four of them bear on a cart, which is what `Q-140` asks about: `Q-135` and `Q-137` both move the master switch (P049), `Q-138` the MRBF rating (P052), and `Q-128`'s shunt has no cart line yet.
