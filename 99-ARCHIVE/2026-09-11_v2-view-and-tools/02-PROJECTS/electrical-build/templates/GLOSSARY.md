...
| **Harness sheets** | `01-DESIGN/diagrams/` — six WireViz drawings rendered by `build` from the tables (D-275, D-276): `HARNESS.svg` the master (every conductor, battery → part → ground), `LEG-L1…L4.svg` one per leg (post housings on the left, each cavity labelled with what feeds it, the conductors bundled per housing, the parts and grounds; L4 carries the backbone and the sill node), `DASH-NODE.svg` the node. Each has an `.html` twin with its bill of materials. Bundles are named by zone and gauge (`LEG-L2 · 14 AWG`, `DASH-NODE · 16 AWG`), housings by their code, devices by their DV id; a wire row names both of its ends |
| **F1–F20** | Fuses. F1–F3, F12, F13, F15, F16, F19, F20 at the dash node · F8, F9, F14 in sealed inline holders at the sill, fitted empty · F17, F18 in the engine bay. F4, F6, F7, F10, F11 were deleted; the numbers are not reused |
