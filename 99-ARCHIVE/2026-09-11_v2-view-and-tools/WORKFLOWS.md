...
rx7 ask                         ← writes ANSWERS.md: every question waiting on you, one file
(type in ANSWERS.md)            ← between the >>> ANSWER and <<< END lines. Plain sentences.
...
You type in exactly three places: **`ANSWERS.md`**, a `set` of a measured number, and `git commit`. Everything else is a workflow.

**`ANSWERS.md` is the only file you ever write prose into.** `rx7 ask` generates it; it holds every question this phase can actually answer, each with one space to type in; `rx7 answers` reads it, applies everything, archives your copy under `99-ARCHIVE/answers/` and deletes the file. Skipping a question costs nothing — a blank one stays open and comes back next time. Everything else you read (`QUESTIONS.md`, `DESIGN.md`, `VIEW.html`, `SHOPPING-LIST.md`) is **generated and rewritten on every build**: if you type in one of those, `build` saves your text as `<name>.typed-in.bak` and tells you, rather than eating it — but it does not become an answer until it is in `ANSWERS.md`.
...
**`rx7 ask`**, then open **`ANSWERS.md`** at the top of the repo. It is a plain file, generated fresh each time, holding only the questions this phase can actually answer — the install checks and measurements wait until the plan reaches them (`rx7 ask --all` overrides). Each one reads on its own: **Ask · Why it matters · Options with consequences · Recommend · Blocks · a one-word ask**, then:
...
Type between the two marker lines. Plain sentences — no `>`, no formatting, any length. `yes`, `no, do (b)`, `follow recommendations`, or a paragraph if it is a brief. Leave the markers alone; they are how each answer finds its question. Then **`rx7 answers`** (or say "I answered questions"). Skip anything you are not ready for — a blank answer stays open and comes back next time.

Behind the scenes `take` moves your text into `data/questions/Q-xxx.md`, which stays the record, and archives your copy under `99-ARCHIVE/answers/`. You never open either. `rx7 answers-list` shows what is answered and not yet applied, and `build` refuses to run while `ANSWERS.md` still holds something unapplied.
