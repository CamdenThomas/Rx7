---
name: rx7-answers
description: Apply every answer Camden wrote in ANSWERS.md (or straight into a packet) — rule or sharpen each, carry it through data, templates, seams, decisions and the build. Trigger on "I answered questions". /rx7-answers [project|all].
---

# /rx7-answers `[project|all]`

Camden answers in `ANSWERS.md` at the repo root — `python tools/rx7.py ask` writes it, he types plain sentences between the `>>> ANSWER` / `<<< END` markers — then says "I answered questions". This routine is the whole job — one pass, one script of edits, one build.

1. **Find every answer:** `python tools/answers.py`. It moves anything in `ANSWERS.md` into the packets first (`tools/inbox.py take`, which archives his copy under `99-ARCHIVE/answers/`), then prints every answered packet. Read them all before touching anything.

   **If it prints 0 and he says he answered, stop and find the text — do not run `build`, and never tell him nothing was answered until you have looked.** In order: `ANSWERS.md` (markers deleted or renamed?), any `*.typed-in.bak` beside a generated document (he typed into `QUESTIONS.md` — `build` saved a copy rather than overwriting), `99-ARCHIVE/answers/` (an earlier take), and `git diff` / `git stash list`. Move what you find into `ANSWERS.md` and take it again. Losing his writing is the worst failure this system has (R12); a wrong ruling is recoverable, a lost session is not.
