...
Questions, decisions, work and the log are data too (system v2):
  data/questions.csv + data/questions/<id>.md     the packets (the record; Camden does not type here)
  ...
  Camden's two commands - the only ones he needs, and the only place he types:
    python tools/rx7.py ask  [project]              write ANSWERS.md: every question waiting on him
    python tools/rx7.py take                        move what he typed into the packets
...
def answered(body: str) -> str:
    ...
    Quoted or not, indented or not, blank line after the label or not: if he typed
    words under the label, they count. This used to require a '>' on every line and
    returned '' in silence when it did not find one, which is how a whole answer
    session was lost on 2026-09-08. Never make this stricter."""
...
def v_packets(db, arg):
    ...
    for q in db.rows("questions", "status = 'open' AND section = ?", sec):
        ...
        # The **ANSWER:** block never renders. A generated file must not contain
        # anything that looks like a place to type - QUESTIONS.md is read-only and
        # build overwrites it. Answers are written in ANSWERS.md (tools/inbox.py).
        body = ANSWER_BLOCK_RE.sub("", db.body("questions", q["id"])).rstrip()
...
ANSWER_HERE = (
    "> **To answer any of these, run `python tools/rx7.py ask`.** It writes `ANSWERS.md` at the top\n"
    "> of the repo - the same questions, each with a space to type in - and `take` moves what you\n"
    "> wrote into the record. This page is generated and is rewritten on every build, so nothing\n"
    "> typed here survives. It is for reading."
)
...
def build(project: Path, views: dict, db: DB, write=True):
    ...
    for tp in sorted((project / "templates").glob("*.md")):
        ...
        for k, l in enumerate(lines):
            if l.startswith("# "):
                lines.insert(k + 1, ""); lines.insert(k + 2, BANNER)
                if outp.name == "QUESTIONS.md":
                    lines.insert(k + 3, ""); lines.insert(k + 4, ANSWER_HERE)
                break
        body = "\n".join(lines) + "\n"
        outputs[outp] = body
    if write:
        for p, body in outputs.items():
            ...
            # Never silently overwrite something a human typed into a generated file.
            if p.exists():
                old = p.read_text(encoding="utf-8")
                for mark in ("**ANSWER:**", ">>> ANSWER "):
                    if mark in old and mark not in body:
                        bak = p.with_name(p.name + ".typed-in.bak")
                        bak.write_text(old, encoding="utf-8", newline="\n")
                        print(f"  !! {p.name} is generated but had an answer typed into it."
                              f" Saved a copy as {bak.name} - answers go in ANSWERS.md.")
                        break
...
def main(argv):
    ...
    if argv[:1] == ["ask"]:
        ...
        return inbox.ask(proj or (rest[0] if rest else None), force="--force" in argv[1:],
                         everything="--all" in argv[1:])
    if argv[:1] == ["take"]:
        import inbox
        inbox.take(); return 0
    if argv[:1] == ["-a"]:
    ...
    if cmd == "tables":
        ...
        if cmd == "build":
            ...
            held = inbox.pending()
            if held:
                print(f"build refused - ANSWERS.md holds {len(held)} typed answer(s) not yet"
                      f" applied: {', '.join(sorted(held))}")
                print("  Building now would regenerate the documents around them. Run first:")
                print("  python tools/rx7.py take     (or say \"I answered questions\")")
                return 1
        problems = run_checks(db, checks, gates)
        for p in problems:
            print("  ✗", p)
