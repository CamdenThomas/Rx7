...
| K-003 | Interval wiper control unit present | Deleted with the factory harness; the interval timer is PMU logic (electrical `logic.csv` WIPER rows, D-225) |  |
...
| K-016 | **Hatch latch switch broken** | Needs sourcing — luxury package `T-033` (LP24); the hatch solenoid itself exists and is wired to on `L4-M 3` (electrical D-274) | Output for the solenoid is `Q-061` → D-180 |
| K-017 | **Fuel-door solenoid — exists** (Camden 2026-09-08; the 2026-09 survey had it as never fitted) | Wired to on `L4-M 4` by the electrical build (D-274); the trigger is the luxury package's (`T-032`, D-180) | Output is `Q-061` → D-180 |
| K-018 | Both retractor manual raise knobs | **Functional** — D-040 holds, no software gesture needed to park the lamps up |  |
| K-020 | **Rear defrost switch (G-24) broken** | **No rebuild impact** — the switch is deleted; O4 is wired to the grid and disabled, and the trigger is the luxury package's panel key (electrical D-210(b), luxury FT16) | Blocks measuring O4 through the switch — jumper the connector or feed the grid directly — the cold read now comes from PMU telemetry at migration (D-174, `T-014` → D-197) |
...
| K-022 | **Washer fluid pump (D-01) not working** | **Unknown until diagnosed** — pump, wiring, or switch? (`T-040`). A genuine pump is in stock (~$48, 2026-09-05 sweep) — replacement closes it if the diagnosis lands on the pump | Diagnose before ordering (`T-039`). The pump also has no PMU output yet — `Q-063` → D-182 |
| K-023 | **Blower motor (G-14) not working** | **Confirmed dead** — replaced in the luxury package onto the PWM blower circuit the electrical build wires now (electrical D-253) | O16 is wired to a dust-capped receptacle (L3-BLW) and DISABLED; its limit is set from the replacement motor spec sheet (D-126) |
| K-024 | **A coolant sensor on the engine side of the thermostat housing was cut by a previous owner.** By the factory diagram it is B-12, the emission system's No.1 water-temperature switch (two wires, LB and LW) — the gauge sender C-02 (one YW wire) is separate and still works | Confirmed 2026-09-07 — left in place as the port's plug, unconnected (electrical D-264). Nothing has read it since the emission control unit left (K-013) |  |
