...
*Rev 2026-08-30 · owns: every fault, quirk and PO hack of this specific car — K-###. One table per area; add a row the moment something is found. Nothing here is verified unless the row says so.*
...
| K-014 | Power mirror control | **Dead.** New heated mirrors with digital control replace it (D-093, `T-031`) | Conductor count `V-060`; dash → sill path `Q-062` |
| K-016 | **Hatch latch switch broken** | Needs sourcing — Checklist 1.7 (`T-033`) | Output for the solenoid is `Q-061` |
| K-017 | **Fuel-door solenoid never existed** | New addition, not a migration — Checklist 1.6 (`T-032`, D-098) | Output is `Q-061` |
| K-018 | Both retractor manual raise knobs | **Functional** — D-040 holds, no software gesture needed to park the lamps up | |
| K-020 | **Rear defrost switch (G-24) broken** | **No rebuild impact** — the switch is deleted; defog moves to the CAN keypad (Checklist 2.18) | Blocks measuring O4 through the switch — jumper the connector or feed the grid directly ([`METER-SESSION.md`](../02-PROJECTS/electrical-pmu/05-BUILD/METER-SESSION.md) Part 3.1) |
...
| K-022 | **Washer fluid pump (D-01) not working** | **Unknown until diagnosed** — pump, wiring, or switch? (`T-040`) | Diagnose before ordering (`T-039`). The pump also has no PMU output yet — `Q-063` |
...
already deleting or replacing. They cost nothing in redesign; what they cost is
**measurement access**, and every one has a workaround in
[`../02-PROJECTS/electrical-pmu/05-BUILD/METER-SESSION.md`](../02-PROJECTS/electrical-pmu/05-BUILD/METER-SESSION.md).
...
| K-007 | Rust — extent not surveyed | Not assessed; `lighting-body/` scope when it is |
