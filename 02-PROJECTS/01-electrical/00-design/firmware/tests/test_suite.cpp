 ...
 *
 * ============================================================
 *  BUILD  (g++: sudo dnf install gcc-c++)
 *    cd .../firmware/tests
 *    g++ test_suite.cpp -o test -std=c++17 -O2
 *    ./test          (or ./run.sh for every suite)
 *
...
static void testStructs() {
    ...
    check(sizeof(icu_body_t)    == 8, "icu_body_t is 8 bytes");
    check(offsetof(icu_body_t, fuel_valid) == 2, "0x218 byte 2 is fuel valid");
    check(offsetof(dcu_climate_t, outside_c) == 5, "0x300 byte 5 is outside-air temp");
    check(offsetof(pmu_outputs_t, out_state) == 2, "0x120 outputs start at byte 2");
    check(SENS_FUEL == 0x20 && CANH_AFR == 0x08, "0x210 fuel-valid b5 and AFR-seen b3");
    ...
    check(sizeof(panel_keys_t)  == 8, "panel_keys_t is 8 bytes");
    check(offsetof(panel_keys_t, held) == 2 && PK_WIN_PASS_DN == 0x100 && PK_COUNT == 10, "0x400: ten keys in bytes 0-1, held in 2-3");
...
static void testStats() {
    ...
    {   /* D-153 on the trip page: nothing recorded is dashes, never a fake number
         * (-128 C rendered as -198 F, 0 psi minimum oil - found by render_pages, X-007) */
        Stats fresh;
        check(!statsHasWater(fresh) && !statsHasOilTemp(fresh) && !statsHasMinOil(fresh) && !statsHasMinVolts(fresh),
              "a fresh trip has no water / oil temp / min oil / min volts to show");
        fresh.maxWaterC = 88; fresh.minOilPress = 180;
        check(statsHasWater(fresh) && statsHasMinOil(fresh), "one reading makes them showable");
    }
...
static void testDiagPage() {
    ...
    /* the channel tag ("O" + up to two digits) must end before the name starts -
     * "O10ACCESSORY" ran together until the X-007 render showed it */
    check(textWidth("O24", 2) <= Cluster::DG_NAME - Cluster::DG_TAG, "channel tag O24 ends before its name");
    check(Cluster::DG_COL0 + Cluster::DG_BAR + Cluster::DG_BAR_W < Cluster::DG_COL1 && Cluster::DG_COL1 + Cluster::DG_BAR + Cluster::DG_BAR_W <= SCR_W, "both diagnostics columns fit side by side on screen");
