...
section inboard, red for tail / brake / turn across the rest.
...
FMVSS 108 S6.4.1: <cite index="17-1">the effective projected luminous lens area of a single compartment stop lamp, and a single compartment rear turn signal lamp, shall be not less than 50 square centimetres (7¾ square inches)</cite>.

And for split designs: <cite index="17-1">if a multiple compartment lamp or multiple lamps are used, the effective projected luminous lens area of each compartment shall be at least 22 square centimetres, provided the combined area is at least 50 square centimetres</cite>.

**Backup lamps have no minimum area** — an NHTSA interpretation states plainly
that <cite index="18-1">there is no minimum EPLLA for these lamps</cite>, though they must still
meet the visibility requirements.

### What that does to a 1 cm strip

`[V-063]` measure the actual stock aperture width. Working from a nominal 30 cm:
...
`[Q-046]` — confirm option C before any strip is bought.
...
`[V-064]` The realistic path is buying **DOT/SAE-compliant LED modules** designed
...
### Design
- [ ] **TL-1** Measure the stock aperture — width, height, depth, mounting `[V-063]`
- [ ] **TL-2** Set strip height from the area math. **Expect ~2 cm**
- [ ] **TL-3** Decide the reverse/red split, recompute the red area
- [ ] **TL-4** Choose LED modules — DOT/SAE compliant, red and white `[V-064]`
- [ ] **TL-5** Design the driver PCB — tail, brake, turn inputs; constant-current out `[Q-046]`
...
- [ ] **TL-10** Measure current draw — update `LOADS.md`
...
### Integrate
- [ ] **TL-17** Migrate on the normal per-circuit loop, `MIGRATION-LOG.md`
...
**Current drops.** LED replaces incandescent — see `LOADS.md`. Update the
...
| ID | Item |
|---|---|
| `V-063` | Stock aperture dimensions — **drives the whole design** |
| `V-064` | DOT/SAE-compliant LED module source, red and white |
| `Q-046` | Driver PCB per side, or simpler diode isolation? |
| `Q-047` | Reverse section width, and does it stay white-only or gain a red function? |
...
*Answers `[Q-047]` and the headlamp half of `[Q-044]`. Added 2026-08.*

## Q-047 · How small can the white reverse section be?
...
## Q-044 · Headlamps — the part that is genuinely different
...
buckets `[V-066 — confirm round vs rectangular on this car]`.
...
`[Q-048]` — which of these three? All are sourceable. The adapter plate for a
...
the sealed-beam figures to whatever the LED unit specifies — update `LOADS.md`
after `T-014`.
