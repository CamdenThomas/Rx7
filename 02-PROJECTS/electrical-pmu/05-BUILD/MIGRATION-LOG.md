...
*Rev 2026-08-30 · owns: the Phase 6 migration order and the per-circuit loop. [`CHECKLIST.md`](CHECKLIST.md) Phase 6 points here rather than restating it. Fill it in the car, at the time — not afterwards.*
...
4. Enable that one output, limit set per the class multiplier in
   [`../01-DESIGN/CHANNEL-SCHEDULE.md`](../01-DESIGN/CHANNEL-SCHEDULE.md) against the `T-014` measurement
5. Operate it. Read live current on the PMU. **Set the soft fuse from measured,
   not estimated** — and type it into `pmu_pins.csv`
...
| 4 | Wipers LOW | O8 | | | | | ☐ | ☐ |
| 5 | Wipers HIGH | O9 | | | | | ☐ | ☐ |
| 6 | Washer pump | **no output yet — `Q-063`** | | | | | ☐ | ☐ |
...
| 19 | Mirror heat (F14) | O15 | | | | | ☐ | ☐ |
| — | ~~Mirror motors~~ | — | deferred until the new mirrors arrive (`Q-062`, `V-060`) | | | | ☐ | n/a |
| 20 | Blower motor | O16 | | | | | ☐ | ☐ |
| 21 | Comfort bus | O15 | | | | | ☐ | ☐ |
| 22 | Hatch release solenoid | **no output yet — `Q-061`** | | | | | ☐ | ☐ |
| 23 | Fuel-door solenoid | **no output yet — `Q-061`** | | | | | ☐ | ☐ |
| 24 | Fuel pump | O5 | | | | | ☐ | ☐ |
| 25 | Ignition / coils | O12 | | | | | ☐ | ☐ |
| 26 | **Start** | O21 / K9 | | | | | ☐ | ☐ |

Rows 6, 22 and 23 cannot be migrated until Checklist 0.23 rules on the two
packets that give them an output.
