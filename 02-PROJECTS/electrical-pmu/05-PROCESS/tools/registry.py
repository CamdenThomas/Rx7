#!/usr/bin/env python3
"""
registry.py — build 05-PROCESS/ID-REGISTRY.md and tools/ids.json from the files.

One row per permanent ID, with its status and where it lives:

    D-###  DECISIONS.md            active / SUPERSEDED BY D-###
    L-###  DECISIONS.md §Lighting  active (merged from lighting-body, D-201)
    Q-###  A-###  V-###            open (OPEN.md) / closed by D-### / closed (no decision cited)
    T-###  TASKS-CAMDEN.md         open / done / cancelled
    K-###  M-###  P-###            00-CAR
...
DEC = rd("02-PROJECTS/electrical-pmu/05-PROCESS/DECISIONS.md")
OPEN = rd("02-PROJECTS/electrical-pmu/05-PROCESS/OPEN.md")
TASKS = rd("02-PROJECTS/electrical-pmu/05-PROCESS/TASKS-CAMDEN.md")
KNOWN = rd("00-CAR/known-issues.md")
MODS = rd("00-CAR/modifications.md")
AUDITS = rd("02-PROJECTS/electrical-pmu/05-PROCESS/AUDITS.md")
...
parse_decisions(DEC, "05-PROCESS/DECISIONS.md", "D")
parse_decisions(DEC, "05-PROCESS/DECISIONS.md §Lighting", "L")
...
for m in re.finditer(r"^\*\*(L-\d{3})\*\*\s*—\s*(.*)$", DEC, re.M):
    for q in re.findall(r"\b([QVA]-\d{3})\b", m.group(2)):
        closer.setdefault(q, m.group(1))
...
for idd in sorted(all_ids):
    if idd in open_e:
        reg[idd] = dict(status="open", title=open_title(OPEN, idd), where="05-PROCESS/OPEN.md")
    elif idd in closer:
        reg[idd] = dict(status=f"closed by {closer[idd]}", title=reg.get(closer[idd], {}).get("title", ""), where="05-PROCESS/DECISIONS.md")
    else:
...
def parse_tasks(text, where):
    for m in re.finditer(r"^\|\s*(~~)?\*?\*?(T-\d{3})\*?\*?(~~)?\s*\|\s*(.*?)\s*\|", text, re.M):
        ...
        if re.search(r"CANCELLED|Cancelled", line): status = "cancelled"
...
parse_tasks(TASKS, "05-PROCESS/TASKS-CAMDEN.md")
...
for m in re.finditer(r"^- \[( |x)\]\s+\*\*(I-\d{2,3})\s*·\s*(?:P\d\s*·\s*)?(.*?)\*\*", AUDITS, re.M):
    reg[m.group(2)] = dict(status="done" if m.group(1) == "x" else "open", title=first_line(m.group(3), 90), where="05-PROCESS/AUDITS.md")
...
names = {"D": "Decisions", "L": "Lighting decisions (deferred scope)", "Q": "Questions", "A": "Assumptions", "V": "Verify items",
...
out = "\n".join(L) + "\n"
target = os.path.join(PROJ, "05-PROCESS", "ID-REGISTRY.md")
...
if CHECK:
    ...
    print(f"registry.py: {len(reg)} IDs -> 05-PROCESS/ID-REGISTRY.md")
