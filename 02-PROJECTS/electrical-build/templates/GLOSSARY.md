...
| **Master harness sheet** | `01-DESIGN/diagrams/HARNESS.svg` (and `.html` with the bill of materials) — every conductor, battery → part → ground, on one WireViz drawing rendered by `build` from the tables (D-275). Bundles are named by zone and gauge (`LEG-L2 · 14 AWG`, `DASH-NODE · 16 AWG`), housings by their code, devices by their DV id; a wire row names both of its ends |
