 ...
 *   - Desktop      (icu_sim/sim_sdl.cpp provides an SDL2 window)
 *
...
public:
    ...
    static const int DG_TAG = 0, DG_NAME = 46, DG_CUR = 164,   /* NAME past "O24" (36 px): X-007 render */
                     DG_LIM = 234, DG_BAR = 298, DG_BAR_W = 200;

    void drawStaticDiag(Framebuffer &fb) {
        fb.clearAll(C_BLACK);
        drawText(fb, "PMU CHANNELS", 18, 16, 3, C_MID);
        drawText(fb, "A     LIMIT", DG_COL0 + DG_CUR, 39, 1, C_DIM);   /* under the title, over the rule (X-007) */
        drawText(fb, "A     LIMIT", DG_COL1 + DG_CUR, 39, 1, C_DIM);
