...
/* Tank capacity for the trip-average calculation. FB is 16.6 gal / ~62.8 L (D-328, from the 1982 brochure S-004). */
static const uint16_t TANK_GAL_X10  = 166;
...
/* Has this stat recorded anything yet? Until it has, the page shows dashes -
 * a missing value never renders as a real number (D-153): -128 C is not a
 * water temperature and 0 psi is not a minimum oil pressure. */
inline bool statsHasWater(const Stats &a)   { return a.maxWaterC   != -128; }
inline bool statsHasOilTemp(const Stats &a) { return a.maxOilTempC != -128; }
inline bool statsHasMinOil(const Stats &a)  { return a.minOilPress != 65535; }
inline bool statsHasMinVolts(const Stats &a){ return a.minVoltsX10 != 65535; }
...
inline void drawTripPage(Framebuffer &fb, const Stats &a, uint16_t mpgX10) {
    ...
    struct Row { const char *label; int value; int dec; const char *unit; bool known; };
    ...
    Row left[10] = {
        { "MAX RPM",     a.maxRpm,          0, "",    true },
        { "MAX SPEED",   a.maxSpeed,        0, "MPH", true },
        { "MAX WATER",   waterF,            0, "F",   statsHasWater(a) },
        { "MAX OIL T",   oilTF,             0, "F",   statsHasOilTemp(a) },
        { "MIN OIL P",   oilPsi,            0, "PSI", statsHasMinOil(a) },
        { "MAX VOLTS",   a.maxVoltsX10,     1, "V",   true },
        { "MIN VOLTS",   minV,              1, "V",   statsHasMinVolts(a) },
        { "DISTANCE",    (int)a.distanceX10,1, "MI",  true },
        { "RUNTIME",     (int)(a.runtimeSec/60), 0, "MIN", true },
        { "IDLE",        (int)(a.idleSec/60),    0, "MIN", true }
    };
    Row right[10] = {
        { "PEAK LAT G",  a.maxLatG,         2, "G",   true },
        { "PEAK ACCEL",  a.maxAccelG,       2, "G",   true },
        { "PEAK BRAKE",  a.maxBrakeG,       2, "G",   true },
        { "MAX CLIMB",   a.maxPitchUp,      0, "DEG", true },
        { "MAX DESCENT", -a.maxPitchDown,   0, "DEG", true },
        { "OVER REDLINE",(int)a.revSeconds, 0, "S",   true },
        { "OVER TEMP",   (int)a.hotSeconds, 0, "S",   true },
        { "LOW OIL",  (int)a.lowOilSeconds, 0, "S",   true },
        { "FUSE TRIPS",  a.softFuseTrips,   0, "",    true },
        { "MPG",         mpgX10,            1, "",    true }
    };
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (left[i].known) drawNum(fb, left[i].value, VX, y, 2, C_GREEN, left[i].dec);
        else               drawText(fb, "---", VX, y, 2, C_DIM);
        if (left[i].unit[0])
            drawText(fb, left[i].unit, VX + 78, y, 2, C_DIM);

        fb.fillRect(RVX, y, 110, 16, C_BLACK);
        drawText(fb, right[i].label, RX, y, 2, C_MID);
        if (right[i].known) drawNum(fb, right[i].value, RVX, y, 2, C_GREEN, right[i].dec);
        else                drawText(fb, "---", RVX, y, 2, C_DIM);
        if (right[i].unit[0])
