...
<script lang="ts">
  ...
  const sub = $derived(
    [[p?.maker, p?.part_no || p?.oem_no].filter(Boolean).join(' ') || (p?.factory === 'yes' ? 'Factory part' : ''), sys?.name].filter(Boolean).join(' · '),
  );
  const top = $derived((p?.specs ?? []).slice(0, 3).map(spec).filter((s) => !!s));
</script>

{#if p}<span class="wrap"><a class="plink" href={href({ name: 'manual', page: 'part', id })}>{label || p.name}</a><span class="peek card" role="tooltip">
    ...
    </span></span>{:else}<span>{label || id}</span>{/if}
